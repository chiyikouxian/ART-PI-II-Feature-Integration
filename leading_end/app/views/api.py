...
@api_bp.route('/ext_cmd/list', methods=['GET'])
def get_ext_cmd_list():
    """获取外部命令列表 (?limit=50&after_seq=0)
    after_seq: 只返回 seq 大于该值的条目，用于增量拉取
    """
    from app.services.tcp_server import get_ext_cmd_list as _get_list
    limit = request.args.get('limit', 50, type=int)
    after_seq = request.args.get('after_seq', 0, type=int)
    data = _get_list(limit, after_seq)
    return jsonify({'code': 0, 'data': data, 'count': len(data)})
...
def update_ai_engine():
    ...
    if 'system_prompt' in data:
        engine.system_prompt = data['system_prompt'].strip()
...
def chat_ai_engine():
    ...
    if engine.system_prompt:
        messages = [{'role': 'system', 'content': engine.system_prompt}] + messages
...
@api_bp.route('/ai-engine/chat-stream', methods=['POST'])
@login_required
def chat_ai_engine_stream():
    """与AI引擎流式对话（SSE），支持讯飞Spark等OpenAI兼容接口"""
    import requests as req_lib
    ...
    if not data or 'id' not in data or 'messages' not in data:
        def _err():
            yield f"data: {json_mod.dumps({'error': '缺少必要参数'})}\n\n"
        return Response(_err(), mimetype='text/event-stream')

    engine = AIEngine.query.get(data['id'])
    if not engine:
        def _err():
            yield f"data: {json_mod.dumps({'error': '引擎不存在'})}\n\n"
        return Response(_err(), mimetype='text/event-stream')
    ...
    def generate():
        try:
            with req_lib.post(url, headers=headers, json=payload,
