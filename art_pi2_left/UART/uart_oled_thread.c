 ...
 */
#define OLED_MAX_COLS       16      /* 128 / 8 = 16字符每行 */
...
/* 显示缓冲区: 3行，每行最多16字符 + '\0' */
static char oled_lines[OLED_MAX_ROWS][OLED_MAX_COLS + 1];
...
static void oled_add_line(const char *line)
{
    ...
    if (oled_line_count >= OLED_MAX_ROWS)
    {
        ...
        {
            rt_memcpy(oled_lines[i], oled_lines[i + 1], OLED_MAX_COLS + 1);
        }
        oled_line_count = OLED_MAX_ROWS - 1;
    }

    /* 复制新行到缓冲区末尾，截断超出OLED_MAX_COLS的部分 */
    rt_strncpy(oled_lines[oled_line_count], line, OLED_MAX_COLS);
    oled_lines[oled_line_count][OLED_MAX_COLS] = '\0';
    oled_line_count++;
