...
static rt_uint8_t          g_head = 0;
static rt_uint8_t          g_tail = 0;
static rt_uint8_t          g_count = 0;
...
void model_queue_reset_partial_sends(void)
{
    ...
    /* Issue 6 fix: capture diagnostic info under the lock and emit LOG_W
     * AFTER re-enabling interrupts. Logging inside rt_hw_interrupt_disable()
     * can block on the UART, which is unacceptable in a critical section. */
