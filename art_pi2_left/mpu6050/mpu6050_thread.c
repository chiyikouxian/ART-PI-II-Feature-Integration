 ...
 *        I2C2 (PE1/PE2): TCA9548A #2 - CH0/CH1: MPU6050, CH2: ICM-20948
 */
...
#define DATA_LOG_HAND_TYPE  "left"

static volatile rt_bool_t data_log_enabled = RT_FALSE;
static volatile rt_bool_t data_test_enabled = RT_FALSE;
...
static rt_err_t icm20948_init_bypass(const char *bus_name)
{
    ...
    /* 扫描bypass模式下I2C总线上的所有设备 */
    rt_kprintf("[ICM-20948] I2C bus scan (bypass mode):\n");
    {
        struct rt_i2c_bus_device *i2c_bus;
        i2c_bus = (struct rt_i2c_bus_device *)rt_device_find(bus_name);
        if (i2c_bus != RT_NULL)
...
void mpu6050_thread_entry(void *parameter)
{
    ...
    for (int ch = 0; ch < MPU6050_I2C2_COUNT; ch++)
    {
        ...
        if (ch == ICM20948_I2C2_CHANNEL)
        {
            ...
            {
                rt_kprintf("  [I2C2-CH%d] Device found! WHO_AM_I=0x%02x\n", ch, who_am_i);

                if (icm20948_init_bypass(MPU6050_I2C2_BUS_NAME) == RT_EOK)
                {
                    rt_kprintf("  [I2C2-CH%d] ICM-20948 bypass mode enabled\n", ch);
    ...
    {
        rt_kprintf("\n[SENSOR] No sensors found! Use 'data_test' for zero-data output.\n");
    }
    ...
    while (1)
    {
        ...
        /* 测试模式输出: 所有传感器数据为0，时间戳和手型不变 */
        if (data_test_enabled)
        {
            rt_uint32_t ts_ms = (rt_tick_get() - data_log_start_tick) * 1000 / RT_TICK_PER_SECOND;
            rt_kprintf("[DATA]%u,%s", ts_ms, DATA_LOG_HAND_TYPE);
            ...
            for (int i = 0; i < 10; i++)
            {
                rt_kprintf(",0,0,0,0,0,0");
            }

            /* dorsal 9轴全0 */
            rt_kprintf(",0,0,0,0,0,0,0,0,0\n");
        }
...
static int data_test(int argc, char **argv)
{
    data_log_start_tick = rt_tick_get();
    data_log_print_header();
    data_log_enabled = RT_FALSE;
    data_test_enabled = RT_TRUE;
    rt_kprintf("[DATA_LOG] Test mode started (%s hand, fixed 90ms interval, all zeros)\n", DATA_LOG_HAND_TYPE);
    return 0;
}
MSH_CMD_EXPORT(data_test, Start serial data logging with all zero sensor data);
