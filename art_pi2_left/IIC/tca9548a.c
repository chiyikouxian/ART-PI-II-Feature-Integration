...
/* IIC引脚操作宏 */
#define TCA_SCL_HIGH()     rt_pin_write(I2C2_SCL_PIN, PIN_HIGH)
#define TCA_SCL_LOW()      rt_pin_write(I2C2_SCL_PIN, PIN_LOW)
#define TCA_SDA_HIGH()     rt_pin_write(I2C2_SDA_PIN, PIN_HIGH)
#define TCA_SDA_LOW()      rt_pin_write(I2C2_SDA_PIN, PIN_LOW)
#define TCA_SDA_READ()     rt_pin_read(I2C2_SDA_PIN)

/* 当前选中的通道 */
static rt_uint8_t current_channel = 0xFF;

/* TCA9548A是否在线 */
static rt_bool_t tca9548a_present = RT_FALSE;
...
rt_err_t tca9548a_init(void)
{
    ...
    /* 探测TCA9548A是否存在: 尝试发送地址，看是否收到ACK */
    TCA_I2C_Start();
    if (TCA_I2C_SendByte((TCA9548A_ADDR << 1) | 0x00) == RT_EOK)
    {
        /* 收到ACK，发送0x00禁用所有通道 */
        TCA_I2C_SendByte(0x00);
        TCA_I2C_Stop();
        tca9548a_present = RT_TRUE;
        rt_kprintf("[TCA9548A] Detected at addr 0x%02x\n", TCA9548A_ADDR);
    }
    else
    {
        TCA_I2C_Stop();
        tca9548a_present = RT_FALSE;
        rt_kprintf("[TCA9548A] Not detected\n");
    }
...
/**
 * @brief  查询TCA9548A是否在线
 * @return RT_TRUE: 存在, RT_FALSE: 不存在
 */
rt_bool_t tca9548a_is_present(void)
{
    return tca9548a_present;
}
