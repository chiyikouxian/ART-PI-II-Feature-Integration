...
static void finsh_thread_entry(void *parameter)
{
    ...
    /* SAY: 命令原始字节缓冲区，用于接收含 UTF-8 中文的语音命令。
     * finsh_getchar() 将 >0x7F 的字节作为负数返回，被 ch<0 过滤丢弃，
     * 所以必须在过滤前拦截原始字节。 */
    static char say_buf[256];
    static int  say_len = 0;
    static int  in_say  = 0;
    ...
    while (1)
    {
        ...
        /* --- SAY: 原始字节拦截 (在 ch<0 过滤之前) --- */
        {
            unsigned char raw = (unsigned char)(ch & 0xFF);
            ...
            if (in_say)
            {
                if (raw == '\r' || raw == '\n')
                {
                    ...
                    if (say_len > 0)
                    {
                        extern rt_err_t vtx316_speak_wait(const char *text);
                        vtx316_speak_wait(say_buf);
                    }
                    say_len = 0;
                    in_say  = 0;
                }
                else if (say_len < (int)sizeof(say_buf) - 1)
                {
                    say_buf[say_len++] = (char)raw;
                }
                continue;
            }
            else
            {
                /* 逐字节匹配 "SAY:" 前缀 */
                const char prefix[] = "SAY:";
                if (raw == (unsigned char)prefix[say_len])
                {
                    say_len++;
                    if (say_len == 4)   /* 完整匹配 "SAY:" */
                    {
                        in_say  = 1;
                        say_len = 0;
                        continue;
                    }
                    /* 前缀字节是 ASCII，ch>=0，让 finsh 正常回显 */
                }
                else
                {
                    say_len = 0;
                    /* 重新检查第一个字节 */
                    if (raw == (unsigned char)prefix[0])
                        say_len = 1;
                }
            }
        }
