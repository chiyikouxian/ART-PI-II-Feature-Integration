...
/**
 * 已适配 ART-PI2 (STM32H7RS + RT-Thread)
...
/**
  ...
  * 返 回 值：无
  */
static void OLED_Delay_us(rt_uint32_t us)
{
	rt_uint32_t delta;
...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	rt_pin_write(OLED_SCL_PIN, BitValue ? PIN_HIGH : PIN_LOW);

	/*如果单片机速度过快，可在此添加适量延时，以避免超出I2C通信的最大速度*/
	OLED_Delay_us(IIC_DELAY_US);
}
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	rt_pin_write(OLED_SDA_PIN, BitValue ? PIN_HIGH : PIN_LOW);
...
/**
  * 函    数：OLED IIC引脚初始化 (供外部模块调用)
  * 参    数：无
  * 返 回 值：无
  * 说    明：初始化SCL和SDA引脚为开漏输出模式
  */
void OLED_I2C_Init(void)
{
	...
	/*释放SCL和SDA*/
	rt_pin_write(OLED_SCL_PIN, PIN_HIGH);
...
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		else					//否则，即多字节字符
		{
			...
			{
				/*检查是否在内置字模库中找到 (未找到时pIndex指向空字符串终止符)*/
				...
				{
					/*内置字模未命中，尝试从TF卡HZK16字库读取*/
					uint8_t gb[2];
					uint8_t hzk_buf[32];
					if (utf8_char_to_gb2312(SingleChar, gb) == 0 &&
						oled_hzk16_get_char(gb[0] - 0xA0, gb[1] - 0xA0, hzk_buf) == 0)
					{
						OLED_ShowImage(X + XOffset, Y, 16, 16, hzk_buf);
					}
					else
					{
						/*HZK16也未找到，显示默认图形（方框问号）*/
						OLED_ShowImage(X + XOffset, Y, 16, 16, OLED_CF16x16[pIndex].Data);
					}
				}
				XOffset += 16;
