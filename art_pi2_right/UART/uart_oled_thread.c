...
static char oled_system_status[OLED_LINE_BUF_SIZE + 1] = "";
static rt_bool_t oled_stt_started = RT_FALSE;
...
void oled_show_system_status(const char *text)
{
    if (text == RT_NULL)
        return;

    /* 如果正在显示初始化状态，忽略此调用 */
    if (init_status_is_showing())
    {
        rt_kprintf("[UART_OLED] Ignoring oled_show_system_status(\"%s\") - init status showing\n", text);
        return;
    }

    rt_strncpy(oled_system_status, text, OLED_LINE_BUF_SIZE);
    oled_system_status[OLED_LINE_BUF_SIZE] = '\0';
...
void oled_show_error_status(const char *text)
{
    oled_show_system_status(text);
}
...
static void oled_render_locked(void)
{
    ...
    if (oled_system_status[0] != '\0')
    {
        OLED_ShowString(0, 8, oled_system_status, OLED_8X16);
        for (i = 0; i < oled_line_count && i < OLED_MAX_ROWS - 1; i++)
        {
            OLED_ShowString(0, 24 + i * OLED_FONT_HEIGHT, oled_lines[i], OLED_8X16);
        }
    }
    else
    {
        for (i = 0; i < oled_line_count && i < OLED_MAX_ROWS; i++)
        {
...
static void oled_add_line(const char *line)
{
    ...
    /* 检查是否只包含空格或不可打印字符 */
    const char *p = line;
    rt_bool_t has_printable = RT_FALSE;
    while (*p != '\0')
    {
        /* 检查是否为可打印字符（ASCII 32-126，或中文UTF-8） */
        if (*p >= 32 && *p <= 126)
        {
            if (*p != ' ' && *p != '\t')
            {
                has_printable = RT_TRUE;
                break;
            }
        }
        else if ((unsigned char)*p >= 0x80)  /* 可能是UTF-8中文 */
        {
            has_printable = RT_TRUE;
            break;
        }
    ...
    if (!has_printable)
    {
        rt_kprintf("[UART_OLED] Ignoring line with no printable chars\n");
        return;  /* 没有可打印字符，忽略 */
    }

    /* 第一次添加有效文本时，退出初始化状态显示 */
    if (init_status_is_showing())
    {
        rt_kprintf("[UART_OLED] First valid text received, exiting init status\n");
        init_status_exit();
    }
...
static void oled_prepare_stt_display(void)
{
    if (oled_stt_started)
        return;

    oled_stt_started = RT_TRUE;
    oled_system_status[0] = '\0';
    oled_line_count = 0;
    rt_memset(oled_lines, 0, sizeof(oled_lines));
}
...
void uart_oled_thread_entry(void *parameter)
{
    ...
    if (uart_dev == RT_NULL)
    {
        ...
        // oled_show_error_status("UART FAIL");  // 由init_status模块统一管理
        return;
    }
    ...
    /* 不再显示初始提示信息，改为由init_status模块统一管理显示 */
