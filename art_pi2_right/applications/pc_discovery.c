/**
 * @file pc_discovery.c
 * @brief Right-hand PC UDP Discovery implementation.
 *
                                       ...
                                       int *out_port)
{
    ...
    /* magic: bytes[0..7] = "ARTPI_PC" (8 bytes) */
...

    {
        ...
        if (endptr == ver_buf || *endptr != '\0')
            return -RT_ERROR;
        if (ver != DISCOVERY_VERSION)
            return -RT_ERROR;
    }

    /* port field: starts after version comma */
...
rt_err_t pc_discovery_start(void)
{
    ...
    rt_strncpy(g_stats.device_name, "right", sizeof(g_stats.device_name) - 1);
