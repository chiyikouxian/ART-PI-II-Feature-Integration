/*
 * imu_notify_thread.c — periodic BLE notification sender for IMU data
 *
...
/* ── Config ───────────────────────────────────────────────────────────────── */
#define NOTIFY_INTERVAL_MS        90     /* ~11.1 Hz, matches serial data_start interval */
...
void imu_notify_set_conn_handle(int32_t conn_handle)
{
    ...
    if (conn_handle < 0)
        _subscribed = RT_FALSE;   /* auto-clear on disconnect */
}
...
static void _notify_entry(void *param)
{
    ...
    while (1) {
        ...
        /* Skip if not connected or client has not subscribed to notifications */
        ...
        pos += rt_snprintf(csv_buf + pos, CSV_BUFFER_SIZE - pos,
                          "[DATA]%u,right,%u", timestamp_ms, frame_seq);
        ...
        int rc = _send_csv_fragments(ch_local, frame_seq, "right", csv_buf, csv_payload_len);
